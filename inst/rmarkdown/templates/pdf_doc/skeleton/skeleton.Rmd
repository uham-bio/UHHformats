...
knitr::write_bib(
  x = c(.packages(), 'bookdown', 'rmarkdown', 'UHHformats',
    ...
    'knitr', 'kableExtra', 'flextable', 'tidyverse'),
...
* `date`: The date; by default set to today's date automatically.
...
* `output`: The nested fields for the output field are based on the arguments of the output function. Since `UHHformats::pdf_doc` is based on `rmarkdown::pdf_document`, see its help page for more options. Current default settings are
...
[flextable](https://ardata-fr.github.io/flextable-book/) creates tables that work well in both PDF and Word output, making it a good choice if you plan to generate both formats:
