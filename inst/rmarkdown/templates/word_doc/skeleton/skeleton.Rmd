...
[flextable](https://ardata-fr.github.io/flextable-book/) creates tables that work well in both PDF and Word output. Table \@ref(tab:flextable1) shows an example:

```{r flextable1}
library(flextable)
df <- mtcars[1:5, 1:6]
flextable(df) |>
  set_caption(caption = "Overview of the first rows and columns in the mtcars dataset.") |>
  autofit() |>
  theme_booktabs() |>
  bold(part = "header")
```
