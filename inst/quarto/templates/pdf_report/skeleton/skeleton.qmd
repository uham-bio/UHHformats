...
- `logo1` and `logo2`: path of the logos that will be displayed on the bottom left and right of title page (after the front cover). Comment out either or both if you do not want a logo.
- `small footer logo`: The small logo shown in the footer of every page is controlled by the file `images/logo.png`. To use a different logo, replace that file with your own (keeping the name `logo.png`). To show no logo at all, simply delete or remove `images/logo.png`.
- `language`: If language is NOT English refer to the file `custom_lang.yml` for automatic text in German. If the document language is another one, adjust this file.
...
If the random variable $Y$ follows a standard normal distribution, i.e. $Y \sim N(0,1)$, its density function can be described with
...
::: {#fig-custom-layout layout="[20,-5,40,-5,30]"}
...
@tbl-kable is an example of using `knitr::kable()` to generate the table and *kableExtra* functions to modify it:

```{r}
#| label: tbl-kable
#| tbl-cap: "A table produced with knitr and kableExtra."
library(kableExtra)
df <- mtcars[1:5, 1:6]
knitr::kable(df, booktabs = TRUE) |>
  ...
  kableExtra::footnote(general = "Your comments go here.")  # note: flextable has also a function named footnote()
```


### The *flextable* package

[flextable](https://ardata-fr.github.io/flextable-book/) creates tables that work well in both PDF and Word output. @tbl-flextable shows an example:

```{r}
#| label: tbl-flextable
#| tbl-cap: "Overview of the first rows and columns in the mtcars dataset."
library(flextable)
df <- mtcars[1:5, 1:6]
flextable(df) |>
  autofit() |>
...
**Tip:** Another excellent table package is [gt](https://gt.rstudio.com/), which provides a tidyverse-compatible grammar for tables.
...
References can be cited directly within the document using the Quarto equivalent of the \LaTeX citation system `[@key]`, where key is the citation key in the first line of the entry in the .bib file. Example: [@Taylor1937]. To cite multiple entries, separate the keys by semicolons (e.g., [@Knupp1999; @Kamm2000]). You can also write in-text citations, as follows: @Taylor1937 say this and that.
