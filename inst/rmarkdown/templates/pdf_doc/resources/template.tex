...
\setmainfont[Path = $fontpath$/, BoldFont = $font_bold$, ItalicFont = $font_italic$, BoldItalicFont = $font_bolditalic$]{$font_regular$}
\newfontfamily\headingfont[Path = $fontpath$/, ItalicFont = $font_bolditalic$]{$font_bold$}
...
\vspace*{5mm}%  push title down
\begin{flushleft}
  \textbf{\color{bluegray}\huge\@title}
  \vskip 0.25em%
  $if(subtitle)$
  {\color{bluegray}\Large $subtitle$}
  $endif$

  \vskip 0.5em%
  $if(author)$
  \textbf{\large\color{bluegray} $author$}\par
  $endif$
  \textbf{\small\color{bluegray}\@date}
\end{flushleft}\egroup
\vskip 1.5em%  space before abstract
}
\makeatother
...
\pagestyle{fancy}
\fancyhf{} % clear all
\fancyhead[R]{\nouppercase{\rightmark}} % show first section name on page
\fancyfoot[R]{\thepage}
\fancyfoot[L]{\raisebox{-2.5mm}{\includegraphics[height=10mm]{logo.png}}}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0pt}

% Make \rightmark track sections (first on page) and ignore subsections
$if(numbersections)$
...
% Section start pages also show header (LaTeX sets \thispagestyle{plain} on \section)
...
% Title page: no header, no header rule
